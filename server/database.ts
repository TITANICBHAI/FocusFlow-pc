...
export function initDatabase(): void {
  const dbPath = join(process.cwd(), 'focusflow.db')
...
export function startFocusSession(session: { taskId: string; startedAt: string; allowedPackages: string[] }): void {
  db.prepare(`UPDATE focus_sessions SET is_active=0, ended_at=? WHERE is_active=1`).run(new Date().toISOString())
  db.prepare(`INSERT INTO focus_sessions (task_id,started_at,is_active,allowed_packages) VALUES (?,?,1,?)`).run(
...
export interface FocusHistoryEntry {
  id: number; taskId: string; taskTitle: string; taskColor: string; taskPriority: string
  startedAt: string; endedAt: string; durationMinutes: number; overrideCount: number
}
...
export function getFocusHistory(days = 90): FocusHistoryEntry[] {
  ...
  return sessions.map(s => {
    ...
    return {
      id: s.id, taskId: s.task_id, taskTitle: s.title ?? 'Unknown Task',
      taskColor: s.color ?? '#6366f1', taskPriority: s.priority ?? 'medium',
      startedAt: s.started_at, endedAt: s.ended_at,
      durationMinutes: durMins, overrideCount: overrideRow?.cnt ?? 0,
    }
  })
}
